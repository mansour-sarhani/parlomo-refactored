...
export const Button = ({
    ...
    if (asChild) {
        ...
        const childExistingClasses = child.props.className ?? "";
        const mergedClasses = `${combinedClasses} ${childExistingClasses}`.trim();
        ...
        if (isDisabled) {
            ...
            childProps["tabIndex"] = props.tabIndex ?? -1;
        }

        return cloneElement(child, childProps, content);
    }
